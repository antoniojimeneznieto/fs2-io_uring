...
public final class liburing {
  ...
  public static int io_uring_wait_cqe_timeout(io_uring ring, io_uring_cqe cqePtr, __kernel_timespec ts) {
		System.out.println("io_uring_wait_cqe_timeout -> io_uring_wait_cqes");
  ...
  public static int io_uring_cq_ready(io_uring ring) {
    MemorySegment cq = io_uring.getCqSegment(ring.segment);
    MemorySegment ktail = io_uring_cq.getAcquireKtail(cq);
    int ktailValue = 0;
    if(!utils.areSegmentsEquals(ktail, MemorySegment.NULL)){ ktailValue = ktail.get(ValueLayout.JAVA_INT, 0L);}
    MemorySegment khead = io_uring_cq.getKhead(cq);
    int kheadValue = 0;
    if(!utils.areSegmentsEquals(khead, MemorySegment.NULL)){ kheadValue = khead.get(ValueLayout.JAVA_INT, 0L);}
    return (ktailValue - kheadValue);
  };

  private static int io_uring_wait_cqe_nr(io_uring ring, io_uring_cqe cqePtr, int waitNr) {
		System.out.println("io_uring_wait_cqe_nr -> __io_uring_get_cqe");
  ...
  public static int io_uring_peek_cqe(io_uring ring, io_uring_cqe cqePtr) {
    if (__io_uring_peek_cqe(ring, cqePtr, MemorySegment.NULL) == 0
      ...
      return 0;
			...
			System.out.println("io_uring_peek_cqe -> io_uring_wait_cqe_nr");
  ...
  public static int io_uring_wait_cqe(io_uring ring, io_uring_cqe cqePtr) {
    if (__io_uring_peek_cqe(ring, cqePtr, MemorySegment.NULL) == 0
      ...
      return 0;
			...
			System.out.println("io_uring_wait_cqe -> io_uring_wait_cqe_nr");
  ...
  public static MemorySegment io_uring_get_sqe(io_uring ring) {
    ...
    int head, next = io_uring_sq.getSqeTail(sq) + 1;
    int shift = 0;
    ...
    if ((flags & constants.IORING_SETUP_SQPOLL) == 0) {
      MemorySegment kheadSeg = io_uring_sq.getKhead(sq);
      if(!utils.areSegmentsEquals(kheadSeg, MemorySegment.NULL)) {
        head = kheadSeg.get(ValueLayout.JAVA_INT, 0L);
      } else {
        head = 0;
      }
    } else {
      MemorySegment kheadSeg = io_uring_sq.getAcquireKhead(sq);
      if(!utils.areSegmentsEquals(kheadSeg, MemorySegment.NULL)) {
        head = kheadSeg.get(ValueLayout.JAVA_INT, 0L);
      } else {
        head = 0;
